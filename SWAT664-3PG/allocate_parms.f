...
!!    LID general
	...
	allocate(rspc_a(mhru))	
      ...
      !! ============================
       ...
       !! SWAT-3PG--------------------------------------------------------------
      allocate (bio_foliage(mhru)) !!Ritesh - dividing forest biomass
      allocate (foliage_incr(mhru))!!Ritesh - daily foliage increment
      allocate (bio_stem(mhru))    !!Ritesh - dividng forest biomass
      allocate (stem_incr(mhru))   !!Ritesh - daily stem increment
      allocate (bio_root(mhru))    !!Ritesh - dividing forest biomass
      allocate (root_incr(mhru))   !!Ritesh - daily root increment
      allocate (bio_stock(mhru))   !!Ritesh - forest stock for 3PG
      allocate (foliage_debt(mhru))!!Ritesh - foliage debt accouting for deciduous forests
      allocate (bio_tree(mhru))    !!Ritesh - each tree biomass for 3PG
      allocate (tree_age(mhru))    !!Ritesh - tree age for 3PG
      allocate (bio_loss_foliage(mhru)) !!Ritesh - foliage turnover 
      allocate (bio_loss_root(mhru)) !!Ritesh - root turnover
      allocate (bio_loss_stem(mhru)) !!Ritesh - stem turnover
      allocate (bio_loss(mhru)) !!cumulative loss for each day (stem, root, leaves)
      allocate (AGB(mhru)) !!Aboveground biomass for each day
      allocate (n_frac_root(mhru)) !!Ritesh - biomass fraction to root
      allocate (n_frac_stem(mhru)) !!Ritesh - biomass fraction to stem
      allocate (n_frac_foliage(mhru)) !!Ritesh - biomass fraction to foliage
      allocate (dbh_tree(mhru))   !!Ritesh - biomass fraction to foliage
      allocate (LAI_3PG(mhru))   !!Ritesh - LAI calculated using 3PG
      allocate (foliage_loss_thinning(mhru))
      allocate (stem_loss_thinning(mhru))
      allocate (root_loss_thinning(mhru))
      allocate (root_loss_age(mhru))
      allocate (stem_loss_age(mhru))
      allocate (foliage_loss_age(mhru))
      allocate (litterfall(mhru))
      allocate (root_turnover(mhru))
      allocate (bio_foliage_ini(mhru)) !!Ritesh - initial daily bio foliage for mass balance evaluation
      allocate (bio_stem_ini(mhru))  !!Ritesh - initial daily stem biomass for mass balance evaluation
      allocate (bio_root_ini(mhru))  !!Ritesh - initial daily root biomass for mass balance evaluation
      allocate (frsd_dormancy_lf(mhru)) !!Ritesh - to track leaves loss in dormancy
      allocate (laimax(mhru)) !!Ritesh - temporary to track laimax
      allocate (biomxyr(mhru)) !!Ritesh - temporaty to track biomxyr
      allocate (delg(mhru))  !!Ritesh - temporary to track heat accumulated each day
      allocate (deltalai(mhru)) !!Ritesh - temporary to track deltalai each day
      allocate (bioday3PG(mhru)) !!Ritesh - temporary to make PAR a global variable 
      allocate (bm_frac(mhru))  !!Ritesh - temporary to use instead of phuacc
      allocate (bio_ms_3PG(mhru))
      allocate (foliage_debt_allocation(mhru))
      allocate (SLA0(mcrdb)) !!Ritesh - SWAT3PG Readin Variables
      allocate (SLA1(mcrdb)) !!Ritesh - SWAT3PG Readin Variables
      allocate (tSLA(mcrdb)) !!Ritesh - SWAT3PG Readin Variables
      allocate (pRx(mcrdb))
      allocate (pRn(mcrdb))
      allocate (CoeffCond(mcrdb))
      allocate (rAge(mcrdb))
      allocate (nAge(mcrdb))
      allocate (pFS2(mcrdb))
      allocate (PFS20(mcrdb))
      allocate (aWs(mcrdb))
      allocate (nWs(mcrdb))
      allocate (gammaF0(mcrdb))
      allocate (gammaF1(mcrdb))
      allocate (tgammaF(mcrdb))
      allocate (gammaR(mcrdb))
      allocate (gammaN0(mcrdb))
      allocate (gammaN1(mcrdb))
      allocate (tgammaN(mcrdb))
      allocate (ngammaN(mcrdb))
      allocate (mR(mcrdb))
      allocate (mS(mcrdb))
      allocate (mF(mcrdb))
      allocate (alphaCx(mcrdb))
      allocate (molPAR_MJ(mcrdb))
      allocate (gDM_mol(mcrdb))
      allocate (thin_power(mcrdb))
      allocate (wSx1000(mcrdb))
      allocate (mort_thinning_key(mcrdb))
      allocate (mort_age_key(mcrdb)) !!Ritesh - SWAT3PG Readin Variables
      allocate (m0(mcrdb))
      allocate (forest_type(mcrdb))
      allocate (SLA(mhru))
      allocate (aH(mcrdb))
      allocate (nHB(mcrdb))
      !! SWAT-3PG ------------------------------------------------------------------
