        subroutine growth_modifier_3PG_FRSD
        !!Ritesh - Module developed to simualte deciduous forest using 3PG
        !!============================================
        !!Input variables
...
!!!First evaluating if there is a foliage debt from last growing season and assigining all assimilated biomass to foliage till the foliage matches last year foliage mass
...
!Calculation of fphys -------------------------------------------------------------------------------------------
            ...
            !Calculation of FR
            !FR = 0.1
            FR = min(strsw(j), strstmp(j), strsn(j))
            ...
            npp_ratio = -(2*10.**(-5)*(tree_age(j)**2)) + 0.0008*tree_age(j) + 0.4656

            
            !creating a npp_ratio check for old age - 
            if (npp_ratio < 0.1) then
                npp_ratio = 0.1
            end if
           ...
           !Ritesh
           bm_frac = 0.
           bm_frac(j) = bio_ms_3PG(j)/1000/250
           !Ritesh
        ...
        !This additional step is performed to allow deciduous trees to gain all leaves in the first month after dormancy
        
        if (foliage_debt(j) > 0.) then
            !foliage_incr(j) = foliage_debt_allocation(j)
            !foliage_incr(j) = (bioday3PG(j))
            foliage_incr(j) = (bioday3PG(j)) + foliage_debt_allocation(j)
            bio_foliage(j) = bio_foliage(j) + foliage_incr(j)
            foliage_debt(j) = foliage_debt(j) - (bioday3PG(j) + foliage_debt_allocation(j))
            !foliage_debt(j) = foliage_debt(j) - (foliage_debt_allocation(j))
         
            
            !!Calculation of LAI based on 3PG
            !SLA = SLA1 + (SLA0 - SLA1) * Exp(-0.69314718*(curyr_mat(j)/tSLA)**2.0)
            !LAI_3PG(j) = SLA*bio_foliage(j)/1000*0.1
