...
!     &        then             
       ...
       !!   surfq_ru(j) = 0.
          ...
          inflpcp = Max(0.,precipday )  !Ritesh - change 1          ! added by Junyu Qi
!        end if
...
!        if (pot_vol(j) < 1.e-6) call etact
        ...
        !! compute crop growth
        call plantmod_forest     !! SWAT-3PG
        

        !! check for dormancy
        if (igro(j) == 1) call dormantF    !! SWAT-3PG
...
!! Srini Pothole
	 ...
	 !! compute reduction in pollutants due to in fixed BMP eff
        ...
        call wetlan                          ! origianl module
       
        !! compute pond processes
        if (ievent == 0) then
           call hrupond                       !! original module
        else
           call hrupondhr
        endif
