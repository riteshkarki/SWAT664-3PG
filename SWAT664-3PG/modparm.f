...
!! By Zhang for C/N cycling
      ...
      !! SWAT-3PG
      integer:: ifor =2  ! =0 default module; =2 SWAT-3PG module 
      character(len=13):: hedbm(17)
      real :: reg !!Modified by Ritesh - making reg a global variable
      real :: par, basal_area, f_phys, m_root !!Ritesh - making par a global variable
      real :: sol_modifier, vpd_modifier, bioday3PG_opt   !!Ritesh - making to global variables
      real :: SLA0_scup, SLA1_scup, pRx_scup, pRn_scup, rAge_scup !!Ritesh - temp variables for SWAT CUP to read param from .bsn
      real :: nAge_scup, pFS2_scup, pFS20_scup, gammaF0_scup !!Ritesh - temp variables for SWAT CUP to read param from .bsn
      real :: gammaF1_scup, tgammaF_scup, gammaR_scup, gammaN0_scup !!Ritesh - temp variables for SWAT CUP to read param from .bsn
      real :: gammaN1_scup, tgammaN_scup, alphaCx_scup !!Ritesh - temp variables for SWAT CUP to read param from .bsn
      
      real, dimension (:), allocatable :: bio_foliage,bio_stem,bio_root !!Ritesh - declaring variables for forest biomass
      real, dimension (:), allocatable :: bio_stock,dbh_tree,bio_tree   !!Ritesh - declaring variabels for 3PG
      real, dimension (:), allocatable :: tree_age,bio_loss_foliage     !!Ritesh - declaring variable for 3PG to track tree age 
      real, dimension (:), allocatable :: bio_loss_root, bio_loss_stem  !!Ritesh - declaring variable for 3PG
      real, dimension (:), allocatable :: bio_loss, AGB
      real, dimension (:), allocatable :: n_frac_root,n_frac_stem  !!Ritesh - declaring variable for 3PG
      real, dimension (:), allocatable :: n_frac_foliage,LAI_3PG  !!Ritesh - declaring variable for 3PG
      real, dimension (:), allocatable :: foliage_debt, foliage_incr !!Ritesh - declaring variables for 3PG
      real, dimension (:), allocatable :: stem_incr, root_incr !!Ritesh - declaring variables for 3PG
      real, dimension (:), allocatable :: foliage_loss_thinning !!Ritesh - declaring variables for 3PG
      real, dimension (:), allocatable :: stem_loss_thinning !!Ritesh - declaring variables for 3PG
      real, dimension (:), allocatable :: root_loss_thinning !!Ritesh - declaring variables for 3PG
      real, dimension (:), allocatable :: root_loss_age,stem_loss_age
      real, dimension (:), allocatable :: foliage_loss_age, litterfall
      real, dimension (:), allocatable :: root_turnover,bio_ms_3PG,SLA
      real, dimension (:), allocatable :: bio_foliage_ini, bio_root_ini !!Ritesh - declaring variables for mass balance evaluation
      real, dimension (:), allocatable :: bio_stem_ini, frsd_dormancy_lf  !!Ritesh - declaring variables for mass balance evaluation
      real, dimension (:), allocatable :: laimax,biomxyr,delg,deltalai !!Ritesh - declaring variables for temporary tracking of variables
      real, dimension (:), allocatable :: bioday3PG,bm_frac !!Ritesh - declaring variables for temporary tracking of variables
      real, dimension (:), allocatable :: SLA0,SLA1,tSLA,pRx,pRn !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: CoeffCond,rAge,nAge,pFS2 !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: pFS20,aWs,nWs,gammaF0,gammaF1 !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: tgammaF,gammaR,gammaN0 !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: gammaN1,tgammaN,ngammaN,mR,mS !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: mF,alphaCx,molPAR_MJ,gDM_mol !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: thin_power,wSx1000 !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: mort_thinning_key !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: mort_age_key,m0,forest_type !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: aH,nHB,foliage_debt_allocation !!Ritesh - declaring variables for reading SWAT-3PG parameter inputs
      real, dimension (:), allocatable :: GPP_day
      !! SWAT-3PG
