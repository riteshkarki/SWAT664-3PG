...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      icrop = idplt(j)
            ...
            !pltfr_p(j) = (pltpfr(1,icrop) - pltpfr(3,icrop)) * (1. - phuacc(j) !!Default - Modified by RItesh to use bm_frac for pltfr_p calculation
      !&   / (phuacc(j) + Exp(bio_p1(icrop) - bio_p2(icrop) * phuacc(j))))
      !&   + pltpfr(3,icrop)
      
      pltfr_p(j) = (pltpfr(1,icrop) - pltpfr(3,icrop)) *(1. - bm_frac(j)
     &   /(bm_frac(j) + Exp(bio_p1(icrop) -bio_p2(icrop) * bm_frac(j))))
     &   + pltpfr(3,icrop)
