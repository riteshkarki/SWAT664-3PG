...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !real :: bm_frac !!Ritesh - converted to global variable
      ...
      !!Ritesh
      !bm_frac = 0.
      !bm_frac = bio_ms_3PG(j)/1000/700
      !!Ritesh
      
      tno3 = 0.
      do l = 1, sol_nly(j)
        tno3 = tno3 + sol_no3(l,j) ! total nitrate in all soil column
      end do
      tno3 = tno3 / n_reduc(j)  !n_reduc is a nitrogen uptake reduction factor - currently defaulted to 300 and reduces tno3.
      ...
      pltfr_n(j) = (pltnfr(1,icrop) - pltnfr(3,icrop)) *(1. - bm_frac(j)                 !fraction of nitrogen on plant
     &         /(bm_frac(j) + Exp(bio_n1(icrop) - bio_n2(icrop) *       
     &         bm_frac(j)))) + pltnfr(3,icrop) 

      un2 = 0.
      un2 = pltfr_n(j) * bio_ms_3PG(j)                                                  !un2 is the amount of nitrogen in plant  
      if (un2 < plantn(j)) un2 = plantn(j)
      uno3d = un2 - plantn(j)
      uno3d = Min(4. * pltnfr(3,icrop) * bioday3PG_opt, uno3d) !Plant nitrogen deficiency for the day - IF this is calculated before the calculation of bioday, bioday for each day is initialized to 0.  Hence, uno3d will always be 0.
      ...
      do l = 1, sol_nly(j)
        ...
        unmx = 0.
        uno3l = 0.
        unmx = uno3d * (1. - Exp(-n_updis * gx / sol_rd)) / uobn    !maximum amount of nitrogen that can be extracted from a soil layer
        uno3l = Min(unmx - nplnt(j), sol_no3(l,j))                  !amount of nitrogen removed from the soil layer
