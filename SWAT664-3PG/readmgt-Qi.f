...
!!    read general management parameters
      ...
      read (109,*) ncrp
      idplt(ihru) = ncrp
      read (109,*) curyr_mat(ihru) !!Ritesh - modified to use input for curyr_mat !laiday(ihru)
...
!!    set values for cover/crop already growing
      ...
      if (igro(ihru) == 1) then
        igrow = 1  
        idplt(ihru) = ncrp  !Ritesh - setting up idplt(ihru) to crop number in crop.dat
        phuacc(ihru) = 0.   !.1  !Ritesh - fraction of plant heat units accumulated. 
	  icr(ihru) = 1
	  icrmx(ihru) = icrmx(ihru) + 1
	  idplrot(icrmx(ihru),ihru) = ncrp              
	  mcrhru(ihru) = mcrhru(ihru) + 1
        !curyr_mat(ihru) = mat_yrs(ncrp) !!Ritesh - curyr_mat(ihru) is read from LAI_INIT
...
!!      read scheduled management practices
     ...
     &          mgt2i, mgt3i, mgt4, mgt5, mgt6, mgt7, mgt8, mgt9, mgt10i
          ...
          ! Ritesh - modified to provide initial stem, root, and foliage biomass for 3PG incorporation
          !select case(idc(idplt(ihru)))
          if(ifor==2) then     
            if(igro(ihru)==1) then
              if (idc(idplt(ihru)) == 7) then
                  if (idplt(ihru) == 7) then 
                      canmx(ihru) = 10.
                      !bio_ms(ihru) = 0.001                       !!units kg/ha
                      bio_stem(ihru) = bio_ms(ihru)*0.65                      !!units kg/ha
                      bio_root(ihru) = bio_ms(ihru)*0.21                     !!units kg/ha
                      bio_foliage(ihru) = bio_ms(ihru)*0.10
                      !foliage_debt(ihru) = bio_ms(ihru)*0.10!!units kg/ha
              
              
                      bio_stock(ihru) = 1347.                      !!units number/ha 
                      !curyr_mat(ihru) = 1                        !modified to read from LAI_INIT above
                      !if (mgt_op > 2) mgt_op = 0
              
                      plantn(ihru) = pltnfr(3, idplt(ihru))*bio_ms(ihru) 
                      plantp(ihru) = pltpfr(3,idplt(ihru))*bio_ms(ihru) 
                      rsdin(ihru) =bio_ms(ihru)*0.05                !! added by Junyu Qi
                  else
                      canmx(ihru) = 10.   
                      !bio_ms(ihru) = 0.001                       !!units kg/ha
                      bio_stem(ihru) = bio_ms(ihru)*0.75                      !!units kg/ha
                      bio_root(ihru) = bio_ms(ihru)*0.21                     !!units kg/ha
                      bio_foliage(ihru) = bio_ms(ihru)*0.04                   !!units kg/ha
                      ...
                      plantn(ihru) = pltnfr(3, idplt(ihru))*bio_ms(ihru) 
                      plantp(ihru) = pltpfr(3,idplt(ihru))*bio_ms(ihru) 
                      rsdin(ihru) =bio_ms(ihru)*0.05
                  end if
              end if
            end if
          end if
        !!units Years!! added by Junyu Qi
              !if( ifor==2) then     
              ! if ( idplt(ihru) == 6 ) then
              !    canmx(ihru) = 10.                            !! added by Junyu Qi
              !
              !    !Initially growing.
              !    !bio_ms(ihru) = 5000.                      !!units kg/ha
              !    !bio_stem(ihru) = 1000.                      !!units kg/ha
              !    !bio_root(ihru) = 2000.                     !!units kg/ha
              !    !bio_foliage(ihru) = 2000.                   !!units kg/ha
              !
              !    !planting initialization
              !    bio_ms(ihru) = 0.001                       !!units kg/ha
              !    bio_stem(ihru) = 0.00025                      !!units kg/ha
              !    bio_root(ihru) = 0.0005                     !!units kg/ha
              !    bio_foliage(ihru) = 0.00025                   !!units kg/ha
              !
              !    bio_stock(ihru) = 545.                      !!units number/ha
              !    !curyr_mat(ihru) = 1                        !!units Years
              !    laiday(ihru) = 0.01            
              !    if (mgt_op > 2) mgt_op = 0
              !    plantn(ihru) = pltnfr(3, idplt(ihru))*bio_ms(ihru) 
              !    plantp(ihru) = pltpfr(3,idplt(ihru))*bio_ms(ihru) 
              ...
              !
              !
              !    bio_stock(ihru) = 1374.                      !!units number/ha 
              !    curyr_mat(ihru) = 0                        !!units Years
              !    !laiday(ihru) = 0.
              !     if (mgt_op > 2) mgt_op = 0
              !    plantn(ihru) = pltnfr(3, idplt(ihru))*bio_ms(ihru) 
              !    plantp(ihru) = pltpfr(3,idplt(ihru))*bio_ms(ihru)
              ...
              !
              !    !Initially growing
              !    !bio_ms(ihru) = 13000.                       !!units kg/ha
              !    !bio_stem(ihru) = 4000.                      !!units kg/ha
              !    !bio_root(ihru) = 5000.                     !!units kg/ha
              !    !bio_foliage(ihru) = 4000.                   !!units kg/ha
              !
              !    !planting initialization
              !    bio_ms(ihru) = 0.001                       !!units kg/ha
              !    bio_stem(ihru) = 0.00025                      !!units kg/ha
              ...
              !    !LAI_3PH(ihru) = 0.001
              !
              !    bio_stock(ihru) = 1347.                      !!units numbers/ha
              !    curyr_mat(ihru) = 0                        !!units Years
              !    !laiday(ihru) = 0.01
              !     if (mgt_op > 2) mgt_op = 0
              !    plantn(ihru) = pltnfr(3, idplt(ihru))*bio_ms(ihru) 
